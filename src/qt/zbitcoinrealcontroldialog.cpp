...
#include "ui_zbitcoinrealcontroldialog.h"
...
std::list<std::string> ZBitcoinrealControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitcoinrealControlDialog::listMints;

ZBitcoinrealControlDialog::ZBitcoinrealControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitcoinrealControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitcoinrealControlDialog::~ZBitcoinrealControlDialog()
{
    delete ui;
}

void ZBitcoinrealControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitcoinrealControlDialog::updateList()
{
...
void ZBitcoinrealControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitcoinrealControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBitcoinReal_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitcoinRealControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitcoinrealControlDialog::GetSelectedMints()
{
...
void ZBitcoinrealControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
