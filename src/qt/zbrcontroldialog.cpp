...
std::set<std::string> ZBrControlDialog::setSelectedMints;
std::set<CMintMeta> ZBrControlDialog::setMints;

ZBrControlDialog::ZBrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBrControlDialog::~ZBrControlDialog()
{
    delete ui;
}

void ZBrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBrControlDialog::updateList()
{
...
void ZBrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBrControlDialog::GetSelectedMints()
{
...
void ZBrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
