...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bitcoinreal address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BR.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bitcoinreal:URI (bitcoinreal:XyZ...?message=example)
            {
...
static void NotifyzBRReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBRReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBRReset.connect(boost::bind(NotifyzBRReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBRReset.disconnect(boost::bind(NotifyzBRReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
