...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZBrStake(spend));
    } else {
        ...
        CBrStake* brInput = new CBrStake();
        brInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(brInput);
    }
