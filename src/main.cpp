...
// XX42    boost::signals2::signal<void(const uint256&)> EraseTransaction;
    ...
    boost::signals2::signal<void(const CBlockLobitcoinrealr&)> SetBestChain;
...
CBlockIndex* FindForkInGlobalIndex(const CChain& chain, const CBlockLobitcoinrealr& lobitcoinrealr)
{
    // Find the first block the caller has in the main chain
    BOOST_FOREACH (const uint256& hash, lobitcoinrealr.vHave) {
...
CoinSpend TxInToZerocoinSpend(const CTxIn& txin)
{
    ...
    std::vector<char, zero_after_free_allobitcoinrealr<char> > dataTxIn;
    ...
    try {
        if ((mode == FLUSH_STATE_ALWAYS) ||
            ...
            if (mode != FLUSH_STATE_IF_NEEDED) {
                g_signals.SetBestChain(chainActive.GetLobitcoinrealr());
            }
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (pblock->GetHash() != Params().HashGenesisBlock() && pfrom != NULL) {
        ...
        if (mi == mapBlockIndex.end()) {
            pfrom->PushMessage("getblocks", chainActive.GetLobitcoinrealr(), uint256(0));
            return false;
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        CBlockLobitcoinrealr lobitcoinrealr;
        uint256 hashStop;
        vRecv >> lobitcoinrealr >> hashStop;

        LOCK(cs_main);

        // Find the last block the caller has in the main chain
        CBlockIndex* pindex = FindForkInGlobalIndex(chainActive, lobitcoinrealr);
        ...
        CBlockIndex* pindex = NULL;
        if (lobitcoinrealr.IsNull()) {
            // If lobitcoinrealr is null, return the hashStop block
            ...
            // Find the last block the caller has in the main chain
            pindex = FindForkInGlobalIndex(chainActive, lobitcoinrealr);
            if (pindex)
                pindex = chainActive.Next(pindex);
        }
    ...
    else if (strCommand == "headers" && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLobitcoinrealr(pindexLast), uint256(0));
        }
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            if (find(pfrom->vBlockRequested.begin(), pfrom->vBlockRequested.end(), hashBlock) != pfrom->vBlockRequested.end()) {
                ...
                pfrom->PushMessage("getblocks", chainActive.GetLobitcoinrealr(), block.hashPrevBlock);
                pfrom->vBlockRequested.push_back(block.hashPrevBlock);
            } else {
                //ask to sync to this block
                pfrom->PushMessage("getblocks", chainActive.GetLobitcoinrealr(), hashBlock);
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && fFetch /*&& !fImporting*/ && !fReindex) {
            ...
            if (nSyncStarted == 0 || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in Bitcoin
                ...
                //pto->PushMessage("getheaders", chainActive.GetLobitcoinrealr(pindexStart), uint256(0));
                pto->PushMessage("getblocks", chainActive.GetLobitcoinrealr(chainActive.Tip()), uint256(0));
            }
        }
