...
CzBRWallet::CzBRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBRSeed_deprecated()) {
                    LogPrintf("%s: Updated zBR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBRCount(nCountLastUsed);
    else if (!walletdb.ReadZBRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBRWallet::Lock()
{
    seedMaster = 0;
}

void CzBRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBRWallet::LoadMintPoolFromDB()
{
...
void CzBRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbrTracker which also adds to database
    pwalletMain->zbrTracker->Add(dMint, true);
...
void CzBRWallet::SeedToZBR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBRWallet::GenerateDeterministicZBR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
