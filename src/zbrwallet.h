...
#ifndef BitcoinReal_ZBRWALLET_H
#define BitcoinReal_ZBRWALLET_H
...
public:
    CzBRWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBR(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BitcoinReal_ZBRWALLET_H
